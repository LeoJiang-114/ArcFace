...
from Arc_Loss.Face_Net import Facenet
...
if __name__ == '__main__':
    ...
    net_path = r"F:\jkl\Arc_Loss\Net_Save\Face_Arc_.pth"
    img_save = r"F:\Reply\ArcFace_Loss\Face_"
    ...
    while True:
        ...
        for i, (data, labels) in enumerate(train_data):
            data, labels = data.to(device), labels.to(device).reshape(batch_size)
            ...
            # plt.clf()
            for i in range(9):
                plt.scatter(f[labels == i, 0], f[labels == i, 1], c=dic[i],s=10)
